...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Invert a) = [(1-r),(1-g),(1-b)]
  where [r,g,b] = rgb a
rgb (Mix c1 c2) = [(r+r2)/2,(g+g2)/2,(b+b2)/2]
  where [r,g,b] = rgb c1
        [r2,g2,b2] = rgb c2
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne rst) = 1 + (fromNat rst)

toNat :: Int -> Maybe Nat
toNat 0 = Just Zero
toNat x
  | x < 0 = Nothing
  | otherwise = case toNat (x-1) of Nothing -> Nothing
                                    Just Zero -> Just (PlusOne Zero)
                                    Just (PlusOne xs) -> Just (PlusOne (PlusOne xs))
...
fromBin :: Bin -> Int
fromBin x = fromBin' 1 0 x

fromBin' :: Int -> Int -> Bin -> Int
fromBin' _ _ End = 0
fromBin' factor acc (O xs) = fromBin' nextFactor acc xs
  where nextFactor = if factor == 1 then 2 else (factor * 2)
fromBin' factor acc (I xs) = factor + (fromBin' nextFactor (acc+factor) xs)
  where nextFactor = if factor == 1 then 2 else (factor * 2)

toBin :: Int -> Bin
toBin 0 = O End
toBin x = toBin' x 0 End

toBin' :: Int -> Int -> Bin -> Bin
toBin' x incrementCount acc = if x == incrementCount
  then acc
  else toBin' x (incrementCount+1) (inc acc)
