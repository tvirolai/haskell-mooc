...
myGcd 0 b = b
myGcd a 0 = a
myGcd a b = if a > b then (myGcd newNumber b) else (myGcd a newNumber)
  where newNumber = if a > b then a-b else b-a
...
countdown' 0 res = res
countdown' n res = countdown' (n-1) (res ++ (show n) ++ "... ")
...
smallestDivisor' x d = if (mod x d == 0) then d else smallestDivisor' x (d+1)
